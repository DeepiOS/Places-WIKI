...
class PlacesViewController: ViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate, HintPresenting {
    ...
    // MARK: - Force Search From Deep-link
    fileprivate var isForceSearchEnabled = true
    ...
    override func viewDidAppear(_ animated: Bool) {
        handleSearchOnReceivedDeepLink(locationName: UserDefaults.standard
            .locationNameReceiveFromDeepLink())
    }
    ...
    func performDefaultSearchIfNecessary(withRegion region: MKCoordinateRegion?) {
        guard currentSearch == nil, !isForceSearchEnabled else {
            return
        }
    ...
    func performDefaultSearch(withRegion region: MKCoordinateRegion?) {
        currentSearch = PlaceSearch(
            ...
            string: nil,
            region: region,
            localizedDescription: WMFLocalizedString("places-search-top-articles", value:"All top articles", comment:"A search suggestion for top articles"), searchResult: nil
        )
    }
    ...
    func handleCompletion(searchResults: [MWKSearchResult], siteURL: URL) -> [PlaceSearch] {
        ...
        updateLocationOnMapIfRequired(withCompletions: completions)
        return completions
    }
...
extension PlacesViewController {
    ...
    fileprivate func handleSearchOnReceivedDeepLink(locationName: String?) {
        ...
        triggerSearch(name)
        // Reset the search string received from deep-link
        UserDefaults.standard.clearLocationNameReceivedFromDeepLink()
    }

    fileprivate func triggerSearch(_ text: String) {
        isForceSearchEnabled = true
        searchBar.text = text
        isWaitingForSearchSuggestionUpdate = true
        NSObject.cancelPreviousPerformRequests(withTarget: self)
        perform(#selector(updateSearchCompletionsFromSearchBarText), with: nil, afterDelay: 0.2)
    ...
    fileprivate func updateLocationOnMapIfRequired(withCompletions completions: [PlaceSearch]) {
        guard isForceSearchEnabled else { return }
        isForceSearchEnabled = false

        guard let placeSearch = completions.first else { return }
        self.searching = false
        currentSearch = placeSearch
