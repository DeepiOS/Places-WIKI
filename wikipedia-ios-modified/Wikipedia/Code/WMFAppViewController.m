...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            NSString *placeNameToSearch = activity.wmf_placeNameToSearch;
            if (articleURL) {
                // For "View on a map" action to succeed, view mode has to be set to map.
                [[self placesViewController] updateViewModeToMap];
                [[self placesViewController] showArticleURL:articleURL];
            }
            if (placeNameToSearch) {
                // Set the place name to user defaults.
                NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
                [standardUserDefaults setLocationNameReceivedFromDeepLink:placeNameToSearch];
                [standardUserDefaults synchronize];

                // For "View on a map" action to succeed, view mode has to be set to map.
                [[self placesViewController] updateViewModeToMap];
                [[self placesViewController] searchPlace:placeNameToSearch];
            }
        } break;
        case WMFUserActivityTypeContent: {
            [self dismissPresentedViewControllers];
